...
export const parseClock = (clock?: { time: number }): Clock =>
  new Clock(clock?.time ?? 6);

export const parsePlayer = (serverPlayer: ServerPlayer): Player =>
  new Player({
    type: serverPlayer.type,
    resources: new ResourceMap(serverPlayer.resources ?? {}),
  });

export const parseTile = (tile: ServerTile): Tile =>
  new Tile({
    row: tile.row,
    column: tile.column,
...
export const parseTiles = (serverTiles: ServerTile[]): Tile[] =>
